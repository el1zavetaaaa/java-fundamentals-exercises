...
public class CrazyStreams {
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(a ->  a.getEmail().split("@")[1]));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .map(a -> a.getFirstName().concat(a.getLastName()).length())
                .reduce(0, (l, i) -> l +i);
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream()
                .sorted(Comparator.comparing(a -> a.getFirstName().concat(a.getLastName())))
                .toList();
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream().collect(Collectors.groupingBy(a -> a.getCreationDate().getMonth(),
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .flatMap(fName -> fName.chars().mapToObj(c -> (char) c))
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                ...
                .flatMap(fullName -> fullName.chars().mapToObj(c -> (char) c))
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }
