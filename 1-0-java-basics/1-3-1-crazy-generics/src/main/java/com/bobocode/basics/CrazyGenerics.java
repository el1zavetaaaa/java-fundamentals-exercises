...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            Optional<? extends BaseEntity> newEntitiesOptional = entities.stream().filter(e -> e.getUuid() == null).findAny();
            return newEntitiesOptional.isPresent();
        }
        ...
        public static <T> Optional<T> findMax(Collection<T> elements, Comparator<? super T> comparator){
            return elements.stream().max(comparator);
        }
        /**
